...
class EventSimulationSpec extends WordSpecLike with Matchers {
  ...
  countries.foreach { country =>
    ...
    for (i <- 1 to 10) {
      val airport = Airport("", "", "", latitude = 0, longitude = 0, countryCode = countryCode, "", "", size = i, baseIncome = 1000, basePopulation = EventSimulation.CANDIDATE_MIN_POPULATION, 0, id = airportId)
